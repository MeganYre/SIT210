...
def main():
    ...
    def apply_lights(*_):
        ...
        def _pct_to_value(var: tk.DoubleVar) -> float:
            return max(0.0, min(100.0, float(var.get()))) / 100.0

        living_led.value = _pct_to_value(living_brightness) if living_state.get() == "on" else 0.0
        bathroom_led.value = (
            _pct_to_value(bathroom_brightness) if bathroom_state.get() == "on" else 0.0
        )
        closet_led.value = _pct_to_value(closet_brightness) if closet_state.get() == "on" else 0.0
